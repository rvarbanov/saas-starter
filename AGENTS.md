...
- **Auth flow:** protected App routes (`/dashboard`, `/dashboard/users`, `/dashboard/coming-soon`, `/dashboard/settings`, `/dashboard/profile`) redirect unauthenticated users to the WorkOS hosted login (OAuth works with the committed `WORKOS_CLIENT_ID`). Completing sign-in needs a real WorkOS test account.
- **Playwright e2e secrets (required):** `E2E_WORKOS_EMAIL`, `E2E_WORKOS_PASSWORD`, `WORKOS_API_KEY`, `WORKOS_COOKIE_PASSWORD`. Local: `.secret`. Cursor Cloud: environment runtime secrets (process env; no `.secret` file needed). GitHub Actions: repo secrets wired in `.github/workflows/ci.yml`. `pnpm test:e2e` fail-fasts if any are missing. Use Playwright **Chromium** (not `channel: "chrome"`) so CI’s Playwright Docker image works without installing Google Chrome.
- **Quality checks** (match CI `quality` job): `pnpm typecheck`, `pnpm lint` (Biome), `pnpm format:check` (Biome format), `pnpm test` (Vitest). E2E: `pnpm exec playwright install chromium --with-deps` once locally, then `pnpm test:e2e` (it starts its own dev server; free port 3000 first).
