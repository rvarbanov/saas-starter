...
- **Quality checks** (match CI `quality` job): `pnpm typecheck`, `pnpm lint` (Biome), `pnpm test` (Vitest). E2E: `pnpm exec playwright install chromium --with-deps` once locally, then `pnpm test:e2e` (it starts its own dev server; free port 3000 first).
